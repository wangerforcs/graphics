...
void Toolbar::layout_mode(Scene& scene)
{
    if (ImGui::BeginTabItem("Layout")) {
        ...
        if (selected_object != nullptr) {
            ...
            quaternionx.w() = cos(radians(x_angle/2));
            quaternionx.x() = sin(radians(x_angle/2));
            quaternionx.y() = 0;
            quaternionx.z() = 0;
            quaterniony.w() = cos(radians(y_angle/2));
            quaterniony.y() = sin(radians(y_angle/2));
            quaterniony.x() = 0;
            quaterniony.z() = 0;
            quaternionz.w() = cos(radians(z_angle/2));
            quaternionz.z() = sin(radians(z_angle/2));
            quaternionz.x() = 0;
            quaternionz.y() = 0;
