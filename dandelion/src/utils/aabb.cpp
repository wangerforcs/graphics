...
{
    // printf("in AABB intersect\n");
    float tmin = std::numeric_limits<float>::lowest();
    float tmax = std::numeric_limits<float>::max();
    // std::cout<<"in AABB intersect"<<std::endl;
    // std::cout<<ray.direction[0]<<" "<<ray.direction[1]<<" "<<ray.direction[2]<<std::endl;
    // std::cout<<dir_is_neg[0]<<" "<<dir_is_neg[1]<<" "<<dir_is_neg[2]<<std::endl;
    for (int i = 0; i < 3; ++i) {
        ...
        if (dir_is_neg[i]) 
            std::swap(t1, t2);
        tmin = std::max(tmin, t1-0.01f);
        tmax = std::min(tmax, t2+0.01f);
    }
