...
void Scene::simulation_update()
{
    ...
    float remained_duration = frame_duration.count();
    ...
    // duration<long int, std::ratio<1, 1000000000> >
    last_update = last_update + duration_cast<std::chrono::nanoseconds>(frame_duration - duration(remained_duration));
}
